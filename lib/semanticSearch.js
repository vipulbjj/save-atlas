...
async function trySemanticSearch(supabase, userId, opts) {
  const { search, category, subcategory, mediaType, collection, igCollection, limit, offset } = opts;
...
function applyIgCollectionFilter(query, igCollection) {
  if (!igCollection) return query;
  return query.contains('ig_collections', [igCollection]);
}
...
async function keywordSearch(supabase, userId, opts) {
  ...
  query = applyIgCollectionFilter(query, igCollection);
