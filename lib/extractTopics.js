...
const STOP = new Set([
  ...
  // pronouns / filler that become junk pills ("These", "Them", "About", "Your Style")
  'these', 'them', 'those', 'that', 'this', 'here', 'there', 'where', 'what', 'which', 'while',
  'your', 'yours', 'mine', 'ours', 'theirs', 'about', 'improve', 'every', 'each', 'some', 'more',
  'most', 'could', 'should', 'being', 'having', 'doing', 'going', 'thing', 'things', 'people',
  'world', 'life', 'year', 'years', 'need', 'look', 'looks', 'come', 'take', 'keep', 'help',
  'made', 'use', 'using', 'used', 'really', 'always', 'never', 'everyone', 'everything',
  'something', 'nothing', 'anything', 'someone', 'through', 'during', 'before', 'without',
  'within', 'between', 'under', 'over', 'onto', 'upon', 'though', 'since', 'until', 'than',
  'then', 'once', 'only', 'own', 'same', 'other', 'such', 'both', 'either', 'neither', 'few',
  'little', 'less', 'least', 'much', 'many', 'still', 'already', 'maybe', 'please', 'thanks',
]);
...
function isWeakThemePhrase(phrase) {
  const words = String(phrase).toLowerCase().split(/\s+/).filter(Boolean);
  if (words.length === 0) return true;
  if (words.some((w) => STOP.has(w))) return true;
  if (words.length === 1 && (words[0].length < 5 || GENERIC.has(words[0]))) return true;
  if (words.length === 2 && words.some((w) => w.length < 4)) return true;
  return false;
}
...
export function buildCategoryFilters(categoryId, subCategoryCounts, rows, limit = 10) {
  ...
  for (const [phrase, count] of Object.entries(phraseFreq)) {
    if (count < 4) continue;
    if (isWeakThemePhrase(phrase)) continue;
  ...
  for (const [word, count] of Object.entries(wordFreq)) {
    ...
    if (isWeakThemePhrase(word)) continue;
  ...
  for (const c of themeCandidates) {
    ...
    if (filters.filter((f) => f.kind === 'theme').length >= MAX_THEMES) break;
